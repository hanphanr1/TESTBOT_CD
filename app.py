...
async def cmd_broadcast(message: types.Message):
    """Admin gửi broadcast đến TẤT CẢ users đã /start bot"""
    if not is_admin(message.from_user.id):
        await message.answer("❌ Bạn không có quyền!")
        return
    ...
    await message.answer(f"📤 Đang gửi thông báo đến {len(users)} users...")
    
    for user in users:
        try:
            await bot.send_message(
                chat_id=user['user_id'],
                text=f"📣 <b>Thông báo từ shop:</b>\n{text}",
                parse_mode=ParseMode.HTML
            )
            success += 1
            await asyncio.sleep(0.03)  # Tránh rate limit
        except Exception as e:
            error_msg = str(e).lower()
            if "blocked" in error_msg or "deactivated" in error_msg:
                blocked += 1
                mark_user_blocked(user['user_id'])
            else:
                failed += 1
...
async def api_broadcast(data: BroadcastRequest, request: Request, auth: bool = Depends(require_admin)):
    """Gửi broadcast đến TẤT CẢ users đã /start bot"""
    users = get_all_bot_users()
    success = 0
    failed = 0
    blocked = 0
